...
IF tab_REG_XML.COUNT > 0 THEN
    ...
    FOR i IN tab_REG_XML.FIRST .. tab_REG_XML.LAST LOOP
        ...
        IF INSTR(tab_REG_XML(i).xml_line, '22.36') > 0 THEN
            v_found_22_36 := TRUE;
            P_LOG('INFO', '[OK] Transaction 22.36 TROUVEE dans le XML',
        ...
        IF INSTR(tab_REG_XML(i).xml_line, '2817') > 0 AND INSTR(tab_REG_XML(i).xml_line, 'OperationNetAmount') > 0 THEN
            v_found_2817 := TRUE;
            P_LOG('INFO', '[OK] Transaction 2817 TROUVEE dans le XML',
                  'LIGNE_XML', TO_CHAR(i) || ' | ' || SUBSTR(tab_REG_XML(i).xml_line, 1, 100),
                  NULL, NULL, v_step);
        END IF;
    END LOOP;
    ...
    IF NOT v_found_22_36 THEN
        P_LOG('WARNING', '[KO] Transaction 22.36 NON TROUVEE dans le XML - Verification necessaire du fichier source',
              NULL, NULL, NULL, NULL, v_step);
    END IF;

    IF NOT v_found_2817 THEN
        P_LOG('WARNING', '[KO] Transaction 2817 NON TROUVEE dans le XML - Verification necessaire du fichier source',
              NULL, NULL, NULL, NULL, v_step);
    END IF;
