-- ============================================================================
-- PR_RN_IMPORT_GESTION.SQL : création du fichier à intégrer dans accurate
-- pour la justification de compte
-- ============================================================================
-- VERSION AVEC TABLE DE LOGS - Remplacement de tous les DBMS_OUTPUT.PUT_LINE
-- ============================================================================
...
-- ============================================================================
-- ETAPE 0 : CREATION DE LA TABLE DE LOGS (A EXECUTER UNE SEULE FOIS)
-- ============================================================================
/*
...
CREATE INDEX IDX_LOG_DT_TYPE ON TA_RN_LOG_EXECUTION(DT_EXECUTION, TYPE_LOG);
CREATE INDEX IDX_LOG_PROCEDURE ON TA_RN_LOG_EXECUTION(NOM_PROCEDURE, DT_EXECUTION);
CREATE INDEX IDX_LOG_CHARGEMENT ON TA_RN_LOG_EXECUTION(ID_CHARGEMENT);

COMMENT ON TABLE TA_RN_LOG_EXECUTION IS 'Table de logs pour le suivi des traitements et exceptions';
COMMENT ON COLUMN TA_RN_LOG_EXECUTION.TYPE_LOG IS 'INFO, DEBUG, WARNING, ERROR, EXCEPTION, REJET';
*/

-- ============================================================================
-- VUE POUR CONSULTATION DES ERREURS (A EXECUTER UNE SEULE FOIS)
-- ============================================================================
/*
CREATE OR REPLACE VIEW V_RN_LOG_ERREURS AS
SELECT ID_LOG, DT_EXECUTION, NOM_PROCEDURE, TYPE_LOG, NOM_BALISE,
       VALEUR_EXTRAITE, MESSAGE, CODE_ERREUR, ID_CHARGEMENT, ETAPE, UTILISATEUR
FROM TA_RN_LOG_EXECUTION
WHERE TYPE_LOG IN ('ERROR', 'EXCEPTION', 'REJET')
...
-- VERSION AVEC LOGS - Janvier 2025
--***********************************************************************
...
-- ============================================================================
c_NOM_PROCEDURE CONSTANT VARCHAR2(50) := 'PR_RN_IMPORT_GESTION_JC';
...
EXCEPTION
    WHEN OTHERS THEN
        -- En cas d'erreur de log, on continue le traitement
...
valeur_balise varchar2(250);
 ...
 BEGIN
    ...
    -- LOG DEBUG : Enregistrement de la valeur extraite
    P_LOG('DEBUG', 'Extraction balise', Balise, valeur_balise);
...
valeur_balise varchar2(128);
 ...
 BEGIN
    ...
    -- LOG DEBUG : Enregistrement de la valeur date extraite
    P_LOG('DEBUG', 'Extraction balise date', Balise, valeur_balise);
...
valeur_balise varchar2(128);
 ...
 BEGIN
    ...
    -- LOG DEBUG : Enregistrement du montant extrait
    P_LOG('DEBUG', 'Extraction balise montant', 'OperationNetAmount', valeur_balise);
...
BEGIN
    ...
    -- LOG DEBUG : Enregistrement de la devise extraite
    P_LOG('DEBUG', 'Extraction balise devise', Balise, valeur_balise);
...
BEGIN
  ...
  P_LOG('INFO', 'Debut du traitement - ID_CHARGEMENT genere', NULL, TO_CHAR(Var_ID_CHARGEMENT_GESTION), NULL, NULL, v_step);
...
EXCEPTION
  ...
  WHEN OTHERS THEN
     P_LOG('EXCEPTION', 'Etape ' || v_step || ': erreur lors du calcul de l''ID_CHARGEMENT  '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE),
		...
		ELSIF (INSTR(LIGNE_FLUX_GESTION,'</Reglement>')) <> 0 THEN
			...
			BEGIN
				...
				IF Var_N_Uncommitted = 500 THEN
					P_LOG('INFO', 'COMMIT intermediaire - 500 enregistrements', NULL, TO_CHAR(Var_N_Uncommitted), NULL, NULL, v_step);
					Var_N_Uncommitted := 0;
					COMMIT;
				END IF;
			EXCEPTION
			   WHEN OTHERS THEN
					P_LOG('EXCEPTION', 'Erreur inconnue lors de l''insertion en TA_RN_IMPORT_GESTION_JC : '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE),
					      'PAYMENTREFERENCE', Var_PAYMENTREFERENCE, TO_CHAR(SQLCODE), DBMS_UTILITY.FORMAT_ERROR_BACKTRACE, v_step);
...
P_LOG('INFO', 'COMMIT final import', NULL, NULL, NULL, NULL, v_step);
COMMIT;
P_LOG('INFO', 'Etape '|| v_step || ': Traitement import dans TA_RN_IMPORT_GESTION_JC termine avec succes', NULL, NULL, NULL, NULL, v_step);

EXCEPTION
WHEN OTHERS THEN
	P_LOG('EXCEPTION', 'Procedure PR_RN_IMPORT_GESTION_JC etape '|| v_step || ': erreur lors de l''import TX_REGLT_GEST --> TA_RN_IMPORT_GESTION_JC '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE),
	      NULL, NULL, TO_CHAR(SQLCODE), DBMS_UTILITY.FORMAT_ERROR_BACKTRACE, v_step);
...
P_LOG('INFO', 'Debut alimentation table export', NULL, NULL, NULL, NULL, v_step);
...
LOOP
	...
	EXCEPTION
	  ...
	  WHEN OTHERS THEN
		P_LOG('EXCEPTION', 'Etape ' || v_step || ': erreur insertion dans TA_RN_EXPORT_JC : '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE),
	...
	EXCEPTION
		...
		WHEN OTHERS THEN
			P_LOG('EXCEPTION', 'Etape '|| v_step || ': erreur insertion TA_RN_EXPORT_JC : '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE),
			      'NUM_COMPTE_ACCURATE', Var_Ref_NUM_COMPTE_ACCURATE, TO_CHAR(SQLCODE), DBMS_UTILITY.FORMAT_ERROR_BACKTRACE, v_step);
			RAISE PB_INSERT_TA_RN_EXPORT_JC;
	END;
...
EXCEPTION
    ...
    WHEN OTHERS THEN
      P_LOG('EXCEPTION', 'Etape '|| v_step || ': erreur alimentation table TA_RN_EXPORT_JC: '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE),
            NULL, NULL, TO_CHAR(SQLCODE), DBMS_UTILITY.FORMAT_ERROR_BACKTRACE, v_step);
...
P_LOG('INFO', 'Debut ecriture fichier de sortie', NULL, NULL, NULL, NULL, v_step);
...
P_LOG('INFO', 'Ecriture fichier de sortie terminee', NULL, NULL, NULL, NULL, v_step);

EXCEPTION
WHEN OTHERS THEN
	P_LOG('EXCEPTION', 'Procedure PR_RN_IMPORT_GEST_JC etape '|| v_step || ': erreur de transfert en TW_EXPORT_GEST_JC '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE),
	      NULL, NULL, TO_CHAR(SQLCODE), DBMS_UTILITY.FORMAT_ERROR_BACKTRACE, v_step);
...
BEGIN
  ...
  P_LOG('INFO', 'Debut generation CLOB ExtraitReglement_JC.txt', NULL, NULL, NULL, NULL, v_step);
  ...
  v_step := 55;
	IF PKG_TEC_FICHIERS.F_ECRIRECSV_CLOB_SILENTLY(t_InfoTrait,
                                         ...
                                         'OUT_APPLI') = -1 THEN
      P_LOG('ERROR', 'Echec generation CLOB ExtraitReglement_JC.txt', NULL, NULL, NULL, NULL, v_step);
  ...
  P_LOG('INFO', 'Generation CLOB ExtraitReglement_JC.txt terminee avec succes', NULL, NULL, NULL, NULL, v_step);

EXCEPTION
  WHEN OTHERS THEN
    EXECUTE IMMEDIATE ('TRUNCATE TABLE TW_EXPORT_GEST_JC');
	P_LOG('EXCEPTION', 'Procedure PR_RN_IMPORT_GEST_JC etape '|| v_step || ': erreur de generation du clob ExtraitReglement_JC.txt en TA_CLOB '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE),
	      NULL, NULL, TO_CHAR(SQLCODE), DBMS_UTILITY.FORMAT_ERROR_BACKTRACE, v_step);
    RAISE PB_GENERATION_CLOB_GEST_JC;
END;
...
/* ****************************************************************************** */
/* OPTION 2 : Conservation de l'historique - DELETE commentes                     */
/* ****************************************************************************** */
BEGIN
v_step := 60;

  P_LOG('INFO', 'Conservation des donnees dans TA_RN_IMPORT_GESTION_JC et TA_RN_EXPORT_JC (historique)', NULL, NULL, NULL, NULL, v_step);

  -- TA_RN_IMPORT_GESTION_JC --
  -- COMMENTÉ POUR GARDER L'HISTORIQUE
  -- DELETE FROM TA_RN_IMPORT_GESTION_JC WHERE ID_CHARGEMENT_GESTION=Var_ID_CHARGEMENT_GESTION;
  -- P_LOG('INFO', 'Nombre de lignes TA_RN_IMPORT_GESTION_JC supprimees', 'ROWCOUNT', TO_CHAR(SQL%ROWCOUNT), NULL, NULL, v_step);

  -- TA_RN_EXPORT_JC --
  -- COMMENTÉ POUR GARDER L'HISTORIQUE
  -- DELETE FROM TA_RN_EXPORT_JC where TA_RN_EXPORT_JC.SOURCE = 'GEST' AND ID_CHARGEMENT=Var_ID_CHARGEMENT_GESTION;
  -- P_LOG('INFO', 'Nombre de lignes TA_RN_EXPORT_JC supprimees', 'ROWCOUNT', TO_CHAR(SQL%ROWCOUNT), NULL, NULL, v_step);

  -- Log du nombre de lignes conservees
  SELECT COUNT(*) INTO Var_N_Uncommitted FROM TA_RN_IMPORT_GESTION_JC WHERE ID_CHARGEMENT_GESTION=Var_ID_CHARGEMENT_GESTION;
  P_LOG('INFO', 'Nombre de lignes TA_RN_IMPORT_GESTION_JC conservees', 'ROWCOUNT', TO_CHAR(Var_N_Uncommitted), NULL, NULL, v_step);

  SELECT COUNT(*) INTO Var_N_Uncommitted FROM TA_RN_EXPORT_JC WHERE SOURCE = 'GEST' AND ID_CHARGEMENT=Var_ID_CHARGEMENT_GESTION;
  P_LOG('INFO', 'Nombre de lignes TA_RN_EXPORT_JC conservees', 'ROWCOUNT', TO_CHAR(Var_N_Uncommitted), NULL, NULL, v_step);

  COMMIT;

  P_LOG('INFO', 'Traitement termine avec succes - Donnees conservees pour historique', NULL, NULL, NULL, NULL, v_step);

EXCEPTION
  WHEN NO_DATA_FOUND THEN
     P_LOG('WARNING', 'Pas de donnees a supprimer : '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE), NULL, NULL, TO_CHAR(SQLCODE), NULL, v_step);

  WHEN OTHERS THEN
    P_LOG('EXCEPTION', 'Etape '|| v_step || ': erreur de suppression des donnees dans TA_RN_IMPORT_GESTION_JC et TA_RN_EXPORT_JC : '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE),
          NULL, NULL, TO_CHAR(SQLCODE), DBMS_UTILITY.FORMAT_ERROR_BACKTRACE, v_step);
...
WHEN PB_LECTURE_TX_REGLT_GEST THEN
 P_LOG('EXCEPTION', 'Erreur lecture donnees XML TX_REGLT_GEST', NULL, NULL, '-20001', NULL, v_step);
...
WHEN SUPPRESSION_IMPOSSIBLE THEN
 ROLLBACK;
 P_LOG('EXCEPTION', 'Erreur critique lors de la suppression de donnees dans TA_RN_IMPORT_GESTION_JC et TA_RN_EXPORT_JC', NULL, NULL, '-20002', NULL, v_step);
...
WHEN PARAM_GESTION_IMPOSSIBLE THEN
 ROLLBACK;
 P_LOG('EXCEPTION', 'Erreur critique en alimentation table gestion_accurate', NULL, NULL, '-20003', NULL, v_step);
...
WHEN PB_INSERT_TA_RN_IMP_GEST_JC THEN
 ROLLBACK;
 P_LOG('EXCEPTION', 'Probleme insertion TA_RN_IMPORT_GESTION_JC', NULL, NULL, '-20004', NULL, v_step);
...
WHEN PB_INSERT_TA_RN_EXPORT_JC THEN
 ROLLBACK;
 P_LOG('EXCEPTION', 'Probleme insertion TA_RN_EXPORT_JC', NULL, NULL, '-20005', NULL, v_step);
...
WHEN PB_TW_EXPORT_GEST_JC THEN
 ROLLBACK;
 P_LOG('EXCEPTION', '*** Probleme alimentation TW_EXPORT_GEST_JC', NULL, NULL, '-20006', NULL, v_step);
 raise_application_error('-20006', 'Problème d''alimentation de la table TW_EXPORT_GEST_JC');

WHEN PB_GENERATION_CLOB_GEST_JC THEN
 ROLLBACK;
 P_LOG('EXCEPTION', '*** Probleme generation CLOB ExtraitReglement_JC', NULL, NULL, '-20007', NULL, v_step);
 raise_application_error('-20007', 'Problème de generation du CLOB ExtraitReglement_JC');

WHEN PB_ID_CHARGEMENT THEN
 ROLLBACK;
 P_LOG('EXCEPTION', 'Probleme lors de la generation ID_CHARGEMENT', NULL, NULL, '-20008', NULL, v_step);
 raise_application_error('-20008','Erreur critique lors de la generation ID_CHARGEMENT');

WHEN PB_RECHERCHE_LIBELLE THEN
 ROLLBACK;
 P_LOG('EXCEPTION', 'Probleme transcodage societe / mode de reglement', NULL, NULL, '-20009', NULL, v_step);
...
WHEN OTHERS THEN
    ...
    ROLLBACK;
	P_LOG('EXCEPTION', 'Erreur : '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE), NULL, NULL, TO_CHAR(SQLCODE), DBMS_UTILITY.FORMAT_ERROR_BACKTRACE, v_step);
