...
-- ============================================================================
-- MODIFICATION TRACE : Variables pour traçage
-- ============================================================================
...
/* ============================================================================ */
/* PROCEDURE P_LOG - Enregistrement dans table de logs                          */
/* IMPORTANT: Doit etre declaree AVANT les fonctions qui l'utilisent            */
/* ============================================================================ */
PROCEDURE P_LOG (
 ...
 BEGIN
	...
	EXCEPTION
	   WHEN NO_DATA_FOUND THEN
		  ...
		  P_LOG('WARNING', 'Mode de reglement NON TROUVE dans TA_RN_MODE_REGLEMENT',
				'CODE_MODE_REGLEMENT', CodeMR,
				NULL, NULL, v_step);

	   WHEN OTHERS THEN
		  --DBMS_OUTPUT.PUT_LINE('Erreur recherche mode de reglement : '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE));
		  P_LOG('ERROR', 'Erreur recherche libelle mode de reglement',
				'CODE_MODE_REGLEMENT', CodeMR,
				SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
...
-- ============================================================================
-- LOG IMMEDIAT AU DEMARRAGE (avant toute autre operation)
-- ============================================================================
BEGIN
    INSERT INTO TA_RN_LOG_EXECUTION (
        DT_EXECUTION, NOM_PROCEDURE, TYPE_LOG, MESSAGE, ETAPE
    ) VALUES (
        SYSTIMESTAMP, 'PR_RN_IMPORT_GESTION_TRACE', 'INFO',
        '>>> SCRIPT DEMARRE - ETAPE 0 <<<', 0
    );
    COMMIT;
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('ERREUR LOG ETAPE 0: ' || SQLERRM);
END;
DBMS_OUTPUT.PUT_LINE('>>> SCRIPT DEMARRE - ETAPE 0 <<<');
-- ============================================================================
...
BEGIN
    INSERT INTO TA_RN_LOG_EXECUTION (
        ...
        SYSTIMESTAMP, 'PR_RN_IMPORT_GESTION_TRACE', 'INFO',
        '>>> ETAPE 10 - Avant SQ_RN_CHRGGEST.nextval <<<', 10
    );
    COMMIT;
EXCEPTION
...
WHEN OTHERS THEN
	 ...
	 P_LOG('ERROR', 'Erreur calcul ID_CHARGEMENT',
		   NULL, 'Etape ' || v_step,
		   SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
...
-- ============================================================================
-- MODIFICATION TRACE : LOG DEBUT TRAITEMENT
-- ============================================================================
...
-- ============================================================================
-- MODIFICATION TRACE : LOG LECTURE XML
-- ============================================================================
...
-- ============================================================================
-- MODIFICATION TRACE : RECHERCHE TRANSACTIONS CIBLES DANS LE XML
-- ============================================================================
...
FOR i IN tab_REG_XML.FIRST .. tab_REG_XML.LAST LOOP
    IF INSTR(tab_REG_XML(i).xml_line, '22.36') > 0 THEN
        ...
        P_LOG('INFO', 'Transaction 22.36 TROUVEE dans le XML',
              ...
              NULL, NULL, 25);
        DBMS_OUTPUT.PUT_LINE('[OK] Transaction 22.36 TROUVEE ligne ' || i);
    END IF;
	...
	IF bSociete = 0 AND bDepAccount = 0 AND bBeneficiary = 0 
		...
		ELSIF (INSTR(LIGNE_FLUX_GESTION,'</Reglement>')) <> 0 THEN
			...
			WHEN OTHERS THEN
				...
				P_LOG('ERROR', 'Erreur recherche formule TA_RN_FORMULES_GESTION',
					  'SOURCE_DEPT', Var_SCHEME || ' / ' || Var_STATUTREGLEMENT,
					  SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
			...
			BEGIN
				...
				-- ============================================================================
				-- MODIFICATION TRACE : LOG DE CHAQUE TRANSACTION INSÉRÉE
				-- ============================================================================
				...
				IF Var_OPERATIONNETAMOUNT = '22.36' THEN
					P_LOG('INFO', '[CIBLE] TRANSACTION CIBLE 22.36 INSEREE',
						  'PAYMENTREF', Var_PAYMENTREFERENCE,
						  NULL, NULL, v_step);
					DBMS_OUTPUT.PUT_LINE('[CIBLE] TRANSACTION CIBLE 22.36 INSEREE | PAYMENTREF=' || Var_PAYMENTREFERENCE || ' | MONTANT=22.36 | CLIENT=' || Var_NUMEROCLIENT);
				ELSIF Var_OPERATIONNETAMOUNT = '2817' THEN
					P_LOG('INFO', '[CIBLE] TRANSACTION CIBLE 2817 INSEREE',
						  'PAYMENTREF', Var_PAYMENTREFERENCE,
						  NULL, NULL, v_step);
					DBMS_OUTPUT.PUT_LINE('[CIBLE] TRANSACTION CIBLE 2817 INSEREE | PAYMENTREF=' || Var_PAYMENTREFERENCE || ' | MONTANT=2817 | CLIENT=' || Var_NUMEROCLIENT);
				END IF;
				-- ============================================================================
			...
			EXCEPTION
			   WHEN OTHERS THEN
					...
					P_LOG('ERROR', 'Erreur insertion TA_RN_IMPORT_GESTION',
						  'PAYMENTREF', Var_PAYMENTREFERENCE,
						  SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
...
-- ============================================================================
-- MODIFICATION 6 : LOG DU COMMIT FINAL
-- ============================================================================
...
WHEN OTHERS THEN
	...
	P_LOG('ERROR', 'Erreur import TX_REGLT_GEST vers TA_RN_IMPORT_GESTION',
		  NULL, 'Etape ' || v_step,
		  SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
...
-- ============================================================================
-- MODIFICATION 7 : LOG DU TRAITEMENT DES COMPTES ACCURATE
-- ============================================================================
...
LOOP
	...
	LOOP
		...
		BEGIN
			...
			-- ============================================================================
			-- MODIFICATION 8 : LOG DU TEST EXISTS AVEC TA_RN_GESTION_ACCURATE
			-- ============================================================================
			DECLARE
			...
			-- ============================================================================
			-- MODIFICATION 9 : LOG APRES INSERT DANS TA_RN_EXPORT
			-- ============================================================================
		...
		EXCEPTION
		  ...
		  P_LOG('INFO', 'Pas de detail pour ce compte', 'COMPTE', TO_CHAR(Var_Ref_ID_COMPTE_BANC_SYST), NULL, NULL, v_step);

		  WHEN OTHERS THEN
		  DBMS_OUTPUT.PUT_LINE('Etape '|| v_step || ': erreur insertion dans TA_RN_EXPORT : '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE));
		  P_LOG('ERROR', 'Erreur insertion TA_RN_EXPORT (non cumule)',
				'COMPTE', TO_CHAR(Var_Ref_ID_COMPTE_BANC_SYST) || ' - ' || Var_Ref_NUM_COMPTE_ACCURATE,
				SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
		...
		EXCEPTION
		  ...
		  P_LOG('INFO', 'Pas de cumul pour ce compte', 'COMPTE', TO_CHAR(Var_Ref_ID_COMPTE_BANC_SYST), NULL, NULL, v_step);

		  WHEN OTHERS THEN
		  DBMS_OUTPUT.PUT_LINE('Etape '|| v_step || ': erreur insertion dans TA_RN_EXPORT : '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE));
		  P_LOG('ERROR', 'Erreur insertion TA_RN_EXPORT (cumul)',
				'COMPTE', TO_CHAR(Var_Ref_ID_COMPTE_BANC_SYST) || ' - ' || Var_Ref_NUM_COMPTE_ACCURATE,
				SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
		  RAISE PB_INSERT_TA_RN_EXPORT;
		...
		IF Var_Ref_GENERATIONCONTREPARTIE='O' THEN
			...
			EXCEPTION
			  WHEN NO_DATA_FOUND THEN
				...
				P_LOG('INFO', 'Pas de contrepartie pour ce compte', 'COMPTE', TO_CHAR(Var_Ref_ID_COMPTE_BANC_SYST), NULL, NULL, v_step);

			  WHEN OTHERS THEN
			     DBMS_OUTPUT.PUT_LINE('Etape ' || v_step || ': erreur insertion dans TA_RN_EXPORT : '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE));
			     P_LOG('ERROR', 'Erreur insertion TA_RN_EXPORT (contrepartie)',
					   'COMPTE', TO_CHAR(Var_Ref_ID_COMPTE_BANC_SYST) || ' - ' || Var_Ref_NUM_COMPTE_ACCURATE,
					   SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
			     RAISE PB_INSERT_TA_RN_EXPORT;
		    END;
...
WHEN NO_DATA_FOUND THEN
	...
	P_LOG('WARNING', 'Curseur_ZonesParCompte vide - aucun compte accurate a traiter', NULL, NULL, NULL, NULL, v_step);

WHEN OTHERS THEN
  DBMS_OUTPUT.PUT_LINE('Etape ' || v_step || ': erreur alimentation table TA_RN_EXPORT : '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE));
  P_LOG('ERROR', 'Erreur alimentation table TA_RN_EXPORT',
		NULL, 'Etape ' || v_step,
		SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
...
WHEN OTHERS THEN
	...
	P_LOG('ERROR', 'Erreur transfert vers TW_EXPORT_GEST',
		  NULL, 'Etape ' || v_step,
		  SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
...
EXCEPTION
  WHEN OTHERS THEN
	...
	P_LOG('ERROR', 'Erreur generation CLOB ExtraitReglement.txt',
		  NULL, 'Etape ' || v_step,
		  SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
...
-- ============================================================================
-- MODIFICATION 10 : DELETE COMMENTÉ pour analyse post-exécution
-- ============================================================================
...
DBMS_OUTPUT.PUT_LINE('DELETE FROM TA_RN_EXPORT commenté pour analyse post-exécution');
-- ============================================================================
...
WHEN NO_DATA_FOUND THEN
	...
	P_LOG('INFO', 'Pas de donnees a supprimer dans TA_RN_EXPORT', NULL, NULL, NULL, NULL, v_step);
...
WHEN OTHERS THEN
  ...
  P_LOG('ERROR', 'Erreur suppression donnees TA_RN_EXPORT',
		NULL, 'Etape ' || v_step,
		SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
...
WHEN NO_DATA_FOUND THEN
	DBMS_OUTPUT.PUT_LINE('Curseur_ZonesParCompte vide : '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE));
	P_LOG('WARNING', 'CurseurC_ZonesParCompte vide - aucun compte TYPE_RAPPRO=C a traiter', NULL, NULL, NULL, NULL, v_step);

WHEN OTHERS THEN
  DBMS_OUTPUT.PUT_LINE('Etape '|| v_step ||': erreur alimentation table TA_RN_EXPORT pour RapCtl : '||TO_CHAR(SQLCODE)||' '||SQLERRM(SQLCODE));
  P_LOG('ERROR', 'Erreur alimentation TA_RN_EXPORT pour RapCtl (TYPE_RAPPRO=C)',
		NULL, 'Etape ' || v_step,
		SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
  RAISE PARAM_GESTION_IMPOSSIBLE;
...
WHEN OTHERS THEN
	...
	P_LOG('ERROR', 'Erreur transfert RapCtl vers TW_EXPORT_GEST',
		  NULL, 'Etape ' || v_step,
		  SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
	RAISE PB_TW_EXPORT_GEST;
END;
...
EXCEPTION
  WHEN OTHERS THEN
	...
	P_LOG('ERROR', 'Erreur generation CLOB ExtraitReglement_RapCtl.txt',
		  NULL, 'Etape ' || v_step,
		  SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
...
-- ============================================================================
-- MODIFICATION 10 : DELETE COMMENTÉS pour analyse post-exécution
-- ============================================================================
-- DELETE FROM TA_RN_EXPORT WHERE TA_RN_EXPORT.SOURCE = 'GEST' AND ID_CHARGEMENT=Var_ID_CHARGEMENT_GESTION;
...
DBMS_OUTPUT.PUT_LINE('DELETE FROM TA_RN_IMPORT_GESTION commenté pour analyse post-exécution');
-- ============================================================================
...
WHEN OTHERS THEN
  ...
  P_LOG('ERROR', 'Erreur suppression donnees TA_RN_IMPORT_GESTION et TA_RN_EXPORT',
		NULL, 'Etape ' || v_step,
		SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
  RAISE SUPPRESSION_IMPOSSIBLE;
END;
...
WHEN PB_LECTURE_TX_REGLT_GEST THEN
 ...
 P_LOG('ERROR', 'EXCEPTION FINALE - Erreur lecture donnees XML TX_REGLT_GEST',
	   NULL, NULL, '-20001', NULL, v_step);
...
WHEN SUPPRESSION_IMPOSSIBLE THEN
 ...
 P_LOG('ERROR', 'EXCEPTION FINALE - Erreur suppression TA_RN_IMPORT_GESTION et TA_RN_EXPORT',
	   NULL, NULL, '-20002', NULL, v_step);
...
WHEN PARAM_GESTION_IMPOSSIBLE THEN
 ...
 P_LOG('ERROR', 'EXCEPTION FINALE - Erreur alimentation table gestion_accurate',
	   NULL, NULL, '-20003', NULL, v_step);
...
WHEN PB_INSERT_TA_RN_IMP_GEST THEN
 ...
 P_LOG('ERROR', 'EXCEPTION FINALE - Probleme insertion TA_RN_IMPORT_GESTION',
	   NULL, NULL, '-20004', NULL, v_step);
...
WHEN PB_INSERT_TA_RN_EXPORT THEN
 ...
 P_LOG('ERROR', 'EXCEPTION FINALE - Probleme insertion TA_RN_EXPORT',
	   NULL, NULL, '-20005', NULL, v_step);
...
WHEN PB_TW_EXPORT_GEST THEN
 ...
 P_LOG('ERROR', 'EXCEPTION FINALE - Probleme alimentation TW_EXPORT_GEST',
	   NULL, NULL, '-20006', NULL, v_step);
...
WHEN PB_GENERATION_CLOB_GST THEN
 ...
 P_LOG('ERROR', 'EXCEPTION FINALE - Probleme generation CLOB ExtraitReglement',
	   NULL, NULL, '-20007', NULL, v_step);
 raise_application_error('-20007', 'Probleme de generation du CLOB ExtraitReglement');
 
WHEN PB_GENERATION_CLOB_GDT THEN
 ROLLBACK;
 DBMS_OUTPUT.PUT_LINE('*** Probleme generation CLOB ExtraitReglement');
 P_LOG('ERROR', 'EXCEPTION FINALE - Probleme generation CLOB ExtraitReglement_RapCtl',
	   NULL, NULL, '-20008', NULL, v_step);
...
WHEN PB_ID_CHARGEMENT THEN
 ...
 P_LOG('ERROR', 'EXCEPTION FINALE - Probleme generation ID_CHARGEMENT',
	   NULL, NULL, '-20009', NULL, v_step);
...
WHEN PB_RECHERCHE_LIBELLE THEN
 ...
 P_LOG('ERROR', 'EXCEPTION FINALE - Probleme transcodage societe / mode de reglement',
	   NULL, NULL, '-20010', NULL, v_step);
...
WHEN OTHERS THEN
	...
	P_LOG('ERROR', 'EXCEPTION FINALE - Erreur non geree dans PR_RN_IMPORT_GESTION',
		  NULL, SQLERRM,
		  SQLCODE, DBMS_UTILITY.FORMAT_ERROR_STACK, v_step);
