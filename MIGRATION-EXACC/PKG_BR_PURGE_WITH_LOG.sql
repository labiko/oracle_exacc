  ...
  *****************************************************************************/
  ...
  BEGIN
    ...
    IF p_deleteReturnsRows = 1 THEN
      -- ===== LOG UTL_FILE MIGRATION =====
      SP_LOG_FICHIER(
          p_package_name      => 'PKG_BR_PURGE',
          p_function_name     => 'PR_PURGE_AUDIT_RECORDS',
          p_directory_oracle  => p_PurgeFilesDir,
          p_file_name         => v_file_name,
          p_file_mode         => 'APPEND',
          p_contenu_apercu    => 'UTL_FILE.FOPEN + PUT_LINE (br_audit.txt)'
      );
      -- ==================================
  ...
  *****************************************************************************/
  ...
  begin
    ...
    if p_deletereturnsrows = 1 then
        ...
        SP_LOG_FICHIER(
            p_package_name      => 'PKG_BR_PURGE',
            p_function_name     => 'PR_PURGE_AUDIT_RECORDS_CA',
            p_directory_oracle  => p_purgefilesdir,
            p_file_name         => v_audit_file_name,
            p_file_mode         => 'APPEND',
            p_contenu_apercu    => 'UTL_FILE.FOPEN (br_audit.txt)'
        );
        SP_LOG_FICHIER(
            p_package_name      => 'PKG_BR_PURGE',
            p_function_name     => 'PR_PURGE_AUDIT_RECORDS_CA',
            p_directory_oracle  => p_purgefilesdir,
            p_file_name         => v_audit_cycle_assign_file_name,
            p_file_mode         => 'APPEND',
            p_contenu_apercu    => 'UTL_FILE.FOPEN (br_audit_cycle_assign.txt)'
        );
        -- ==================================
  ...
  *****************************************************************************/
  ...
  BEGIN
    ...
    IF p_deleteReturnsRows = 1 THEN
      ...
      SP_LOG_FICHIER(
          p_package_name      => 'PKG_BR_PURGE',
          p_function_name     => 'PR_PURGE_DATA_RECORDS',
          p_directory_oracle  => p_PurgeFilesDir,
          p_file_name         => v_file_name,
          p_file_mode         => 'APPEND',
          p_contenu_apercu    => 'UTL_FILE.FOPEN + PUT_LINE (br_data.txt)'
      );
      -- ==================================
  ...
  *****************************************************************************/
  ...
  BEGIN
    ...
    if p_deleteReturnsRows = 1 then
        ...
        SP_LOG_FICHIER(
            p_package_name      => 'PKG_BR_PURGE',
            p_function_name     => 'PR_PURGE_NOTE_GROUP_HOLDINGS',
            p_directory_oracle  => p_PurgeFilesDir,
            p_file_name         => v_file_name,
            p_file_mode         => 'APPEND',
            p_contenu_apercu    => 'UTL_FILE.FOPEN + PUT_LINE (br_note_group_holding.txt)'
        );
        -- ==================================
  ...
  *****************************************************************************/
  ...
  BEGIN
    ...
    IF p_deleteReturnsRows = 1 THEN
      ...
      SP_LOG_FICHIER(
          p_package_name      => 'PKG_BR_PURGE',
          p_function_name     => 'PR_PURGE_NOTE_GROUP',
          p_directory_oracle  => p_PurgeFilesDir,
          p_file_name         => v_file_name,
          p_file_mode         => 'APPEND',
          p_contenu_apercu    => 'UTL_FILE.FOPEN + PUT_LINE (br_note_group.txt)'
      );
      -- ==================================
  ...
  *****************************************************************************/
  ...
  BEGIN
    ...
    if (p_deleteReturnsRows = 1) then
        ...
        SP_LOG_FICHIER(
            p_package_name      => 'PKG_BR_PURGE',
            p_function_name     => 'PR_PURGE_HOLDING_NOTES',
            p_directory_oracle  => p_PurgeFilesDir,
            p_file_name         => v_file_name,
            p_file_mode         => 'APPEND',
            p_contenu_apercu    => 'UTL_FILE.FOPEN + PUT_LINE (br_holding_notes.txt)'
        );
        -- ==================================
  ...
  *****************************************************************************/
  ...
  BEGIN
    ...
    IF p_deleteReturnsRows = 1 THEN
      ...
      SP_LOG_FICHIER(
          p_package_name      => 'PKG_BR_PURGE',
          p_function_name     => 'PR_PURGE_CHECKPOINT_AUDITS',
          p_directory_oracle  => p_PurgeFilesDir,
          p_file_name         => v_file_name,
          p_file_mode         => 'APPEND',
          p_contenu_apercu    => 'UTL_FILE.FOPEN + PUT_LINE (br_checkpoint_audit.txt)'
      );
      -- ==================================
  ...
  *****************************************************************************/
  ...
  BEGIN
    ...
    IF p_deleteReturnsRows = 1 THEN
      ...
      SP_LOG_FICHIER(
          p_package_name      => 'PKG_BR_PURGE',
          p_function_name     => 'PR_PURGE_CHECKPOINTS',
          p_directory_oracle  => p_PurgeFilesDir,
          p_file_name         => v_file_name,
          p_file_mode         => 'APPEND',
          p_contenu_apercu    => 'UTL_FILE.FOPEN + PUT_LINE (br_checkpoints.txt)'
      );
      -- ==================================
  ...
  *****************************************************************************/
  ...
  BEGIN
    ...
    IF p_deleteReturnsRows = 1 THEN
      ...
      SP_LOG_FICHIER(
          p_package_name      => 'PKG_BR_PURGE',
          p_function_name     => 'PR_PURGE_DATA_EXTRA_RECORDS',
          p_directory_oracle  => p_PurgeFilesDir,
          p_file_name         => v_file_name,
          p_file_mode         => 'APPEND',
          p_contenu_apercu    => 'UTL_FILE.FOPEN + PUT_LINE (br_data_extra.txt)'
      );
      -- ==================================
      file_id2 := UTL_FILE.FOPEN(p_PurgeFilesDir,v_file_name,'a',32767);
...
-- ===== SECTION INITIALISATION PACKAGE =====
BEGIN
    SP_LOG_FICHIER(
        p_package_name      => 'PKG_BR_PURGE',
        p_function_name     => 'INIT',
        p_directory_oracle  => NULL,
        p_file_name         => NULL,
        p_file_mode         => 'INIT',
        p_contenu_apercu    => 'PACKAGE CHARGE EN MEMOIRE'
    );
-- ============================================
end pkg_br_purge;