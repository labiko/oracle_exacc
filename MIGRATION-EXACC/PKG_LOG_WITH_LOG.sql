...
-- #
-- # VERSION AVEC INSTRUMENTATION SP_LOG_FICHIER POUR MIGRATION ExaCC
-- # Seule la fonction F_ECRIRE_LOG est instrumentee (UTL_FILE)
-- #
-- # MODIFICATIONS :
-- # ---------------------------------------------------------------------
...
-- # 2.0     | 23/02/2026 |           | ALPHA  | Ajout SP_LOG_FICHIER ExaCC
-- ***********************************************************************
...
-- Liste des types de message de log a tracer
...
-- #   + s_DirComplet : Nom complet de l'objet directory oracle recherche
-- #   + b_Existe     : booleen : 1=Vrai, 0=Faux
-- # MODIFICATIONS :
-- # ---------------------------------------------------------------------
-- # Version | Date       | Reference | Auteur | Specification
-- # ------- | ---------- | --------- | ------ | -------------------------
...
BEGIN
    ...
    -- Si le directory recherche n'est pas renseigne
    IF trim(s_DirComplet) IS NULL THEN
        PKG_LOG.P_ECRIRE(t_InfoTrait,
                         PKG_LOG.gt_LOG_TYPE_ERR,
                         'ERREUR : Directory recherche non renseigne. Impossible de verifier son existence !',
                         1,
                         s_FONCTION);
        RETURN 1;
    END IF;
...
-- # DESCRIPTION  : Lire le chemin unix correspondant a un objet directory oracle
-- # PARAMETRES   :
-- #   + t_InfoTrait   : informations du traitement
...
BEGIN
    ...
    -- Initialiser le chemin reccherche a vide
    s_DirUnixPath:='';
...
-- # PARAMETRES   :
-- #   + t_InfoTrait  : informations du traitement
-- #   + s_DirDemande : Type de repertoire demande : DIR_TEMP, DIR_IN, DIR_OUT, DIR_LOG
-- #   + s_DirComplet : Nom complet du directory oracle retourne
-- #   + s_DirUnix    : Chemin unix equivalent au directory Oracle
-- # MODIFICATIONS :
-- # ---------------------------------------------------------------------
-- # Version | Date       | Reference | Auteur | Specification
-- # ------- | ---------- | --------- | ------ | -------------------------
...
BEGIN
    ...
    -- Si le directory demande n'est pas renseigne
    ...
    -- Si le directory demande ne contient pas la liste de valeurs suivantes :
    ...
    IF (
       ...
       AND INSTR(s_DirDemande,'DIR_LOG')  = 0
        ...
        PKG_LOG.P_ECRIRE(t_InfoTrait,
                         PKG_LOG.gt_LOG_TYPE_ERR,
                         'ERREUR : Directory demande incorrect. ' ||
    ...
    -- Selon le contenu du directory demande
    ...
    -- Verifier l'existence du directory
    ...
    -- Ce plan B n'est mis en place qu'a titre temporaire
    IF b_Existe=0 THEN

        -- Se rabbattre sur les objets directory
        -- independant du code appli
        IF INSTR(s_DirDemande,'DIR_TEMP') <> 0 THEN
        ...
        -- Si le directory n'existe pas non plus : constat d'echec
        IF b_Existe=0 THEN
            PKG_LOG.P_ECRIRE(t_InfoTrait,
                             PKG_LOG.gt_LOG_TYPE_ERR,
...
-- # DESCRIPTION  : Afficher un message a l'ecran (DBMS_OUTPUT)
-- #                sur une ou plusieurs lignes selon la taille
-- #                du message
-- # PARAMETRES   :
-- #   + ps_Message        : Texte du message a afficher
-- # MODIFICATION
-- # ---------------------------------------------------------------------
-- # Version | Date       | Reference | Auteur | Specification
-- # ------- | ---------- | --------- | ------ | -------------------------
...
IS
    ...
    -- Texte du message debarrasse de ses espaces avant et apres
    s_Msg      VARCHAR2(4096):='';

    -- Texte d'une ligne de message
    s_TexteLigne   VARCHAR2(255):='';

    -- Nombre de lignes necessaire pour afficher le message
    i_NbLignes     PLS_INTEGER := 0;

    -- Nombre de caracteres restant sur la derniere ligne
    i_Reste        PLS_INTEGER := 0;

    -- No de ligne courante (boucle)
    i_NoLigneCour  PLS_INTEGER := 1;

    -- Position (No de caractere) dans le message
...
BEGIN
    ...
    -- Si le message est de taille inferieur la taille max d'une ligne
    ...
    -- Calcul du nombre de lignes necessaire pour afficher le message
    i_NbLignes:=LENGTH(s_Msg)/i_TAILLE_MAX_LIGNE;

    -- Calcul du nombre de caracteres restant sur la derniere ligne
    i_Reste:=MOD(LENGTH(s_Msg),i_TAILLE_MAX_LIGNE);

    -- Si la derniere ligne du message n'est pas une ligne complete
    IF i_Reste > 0
    THEN
        -- Incrementer le nombre de lignes necessaire
        -- pour afficher le message
        i_NbLignes:=i_NbLignes+1;
    END IF;

    -- Pour chaque ligne a afficher
    FOR i_NoLigneCour IN 1..i_NbLignes LOOP

        -- Extraire le texte de la ligne a afficher
        -- a partir du message original
        s_TexteLigne:=SUBSTR(s_Msg,i_PosDebLig,i_TAILLE_MAX_LIGNE);

        -- Afficher la ligne
        DBMS_OUTPUT.PUT_LINE(s_TexteLigne);

        -- Mettre a jour la position dans le message
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        -- Mauvaise idee d'afficher un message avec DBMS_OUTPUT
        -- quand l'erreur est due a la saturation du buffer de DBMS_OUTPUT !!
...
-- # PARAMETRES   :
-- #   + ps_UserOracle       : Identifiant utilisateur Oracle connecte
...
IS
    ...
    -- Position (nb de caracteres)
    -- dans la chaine constituant la pile des appels
    n_Position NUMBER:=0;

    -- Indicateur de debut des donnees utiles
...
BEGIN
    ...
    LOOP
        ...
        -- Rechercher la position de la fin de la 1ere ligne
        -- de la pile des appels
        n_Position := INSTR( s_CallStack, CHR(10) );

        -- Condition de sortie de la boucle :
        -- * le niveau souhaite dans la pile est atteind OU
        ...
        -- Si le debut des donnees utiles n'a pas encore ete trouve
        IF ( NOT b_DebPileTrouve ) THEN

            -- Si la ligne courante correspond a la derniere ligne de titre
            IF ( s_LigneCour LIKE '%handle%number%name%' ) THEN

                -- Fixer a vrai l'indicateur de debut des donnees utiles
                b_DebPileTrouve := TRUE;

            END IF;

        -- Cas des lignes de donnes utiles
        -- (les lignes de titre ont ete passees)
        ELSE
            ...
            -- Incrementer le niveau courant dans la pile
            n_NiveauPileCour := n_NiveauPileCour + 1;

            -- Afficher la pile
            P_AFFICHER(SUBSTR(s_CallStack,1,255));

            -- S'il s'agit de la procedure appelante
            IF ( n_NiveauPileCour = n_NIVEAU_PILE_THEIR_CALLER ) THEN
                ...
                -- Determiner le type de l'objet appelant
...
IS
    ...
    -- Informations sur le contexte formatees correctement
    s_Contexte VARCHAR2(64):='';

    -- Utilisateur Oracle connecte
    s_UserOracle VARCHAR2(15):='';

    -- Nom de l'objet appelant (package, procedure...)
...
BEGIN
    ...
    -- Retourner les informations formatees
    RETURN s_Contexte;
...
-- #   + pn_CdException   : Code exception a l'origine du message
-- #   + ps_LbMessage     : Libelle du message
...
IS
    ...
    -- Date/Heure d'ecriture du message
    d_DateMessage TIMESTAMP DEFAULT SYSTIMESTAMP;

    -- Libelle du type de message
...
BEGIN
    ...
    -- Lire le nouvel identifiant de message (sequence)
    SELECT
    ...
    -- Lire le libelle du type de message
    -- a partir du code type de message
    SELECT
            L.LB_TYPE_MESSAGE
    INTO
            s_LbTypeMessage
    ...
    -- Afficher tout le message et ses elements associes
    P_AFFICHER('---------------------------------------------');
...
EXCEPTION
    ...
    WHEN OTHERS THEN
        ...
        -- Annuler la transaction autonome --> non, ce doit etre fait au niveau du prog appelant : faux!!
        ROLLBACK;
...
-- #   + s_LbMessage   : Libelle du message
-- #   + n_CdException : Code de l'exception a l'origine du message
-- #   + s_IdProg      : Identifiant du programme a l'origine du message
-- # MODIFICATIONS :
-- # ---------------------------------------------------------------------
-- # Version | Date       | Reference | Auteur | Specification
-- # ------- | ---------- | --------- | ------ | -------------------------
...
BEGIN
    ...
    -- Lire l'identifiant d'execution (ou de declenchement) du traitement
    ...
    -- Extraire eventuellement le No de ligne des informations
    -- sur le programme passees en parametre sous le format
    -- <User Oracle>.<Objet>@<No de ligne>
...
-- # DESCRIPTION  : Mettre a jour les types de message log a tracer
-- # PARAMETRES   :
-- #   + ps_ListeTypeMsgLog : Liste des type de message log a tracer
-- # MODIFICATIONS :
-- # ---------------------------------------------------------------------
-- # Version | Date       | Reference | Auteur | Specification
-- # ------- | ---------- | --------- | ------ | -------------------------
...
BEGIN
    ...
    -- Mettre a jour la liste des type de message
...
-- # DESCRIPTION  : Lire les types de message log a tracer
-- # PARAMETRES   :
-- #   + ps_ListeTypeMsgLog : Liste des type de message log a tracer
-- # MODIFICATIONS :
-- # ---------------------------------------------------------------------
...
/******************************************************************************
   NAME:    F_ECRIRE_LOG
   PURPOSE: Ecrit le resultat de la requete passee en parametre, dans un
            fichier passe en parametre.
   PARAMS : p_query     : requete
            p_separator : format du separateur dans le fichier
            p_dir       : repertoire de destination du fichier
   ...
   2.0        23/02/2026  ALPHA            Ajout SP_LOG_FICHIER pour migration ExaCC
******************************************************************************/
...
IS
    ...
    -- Compteur des colonnes ecrites dans le fichier de log
    n_ColCnt        NUMBER DEFAULT 0;

    -- Compteur des lignes ecrites dans le fichier de log
    n_Cnt           NUMBER DEFAULT 0;

    -- Chemin unix d'ecriture du fichier de log
    s_DirUnix       VARCHAR2(255):='';

    -- Nom de l'objet Directory Oracle correspondant au repertoire unix
    -- d'ecriture du fichier de log
    s_DirComplet    VARCHAR2(30):='';

    -- Variable pour le nom de fichier reel (avec ou sans prefixe #)
    v_RealFilename  VARCHAR2(500):='';
...
BEGIN
    ...
    IF UPPER(s_mode) = 'W' THEN
       -- On prefixe le nom de fichier avec le caractere #
       -- afin d'eviter qu'il ne soit recupere par une autre application
       -- avant d'etre totalement ecrit
       v_RealFilename := '#'||s_Filename;
    ...
    -- =====================================================================
    -- INSTRUMENTATION ExaCC : Log OPEN
    -- =====================================================================
    SP_LOG_FICHIER(
        p_package_name      => 'PKG_LOG',
        p_function_name     => 'F_ECRIRE_LOG',
        p_directory_logique => s_Dir,
        p_directory_oracle  => s_DirComplet,
        p_file_name         => v_RealFilename,
        p_file_mode         => s_Mode,
        p_statut            => 'OPEN'
    );
    -- =====================================================================
    ...
    -- Lire le curseur et completer le fichier de log
    LOOP
    ...
    -- Forcer l'ecriture du fichier de log sur le disque
    ...
    -- =====================================================================
    -- INSTRUMENTATION ExaCC : Log OK (succes)
    -- =====================================================================
    SP_LOG_FICHIER(
        p_package_name      => 'PKG_LOG',
        p_function_name     => 'F_ECRIRE_LOG',
  ...
  EXCEPTION
     WHEN  UTL_FILE.INVALID_PATH THEN
        -- =====================================================================
        -- INSTRUMENTATION ExaCC : Log ERROR
        -- =====================================================================
        SP_LOG_FICHIER(
            p_package_name      => 'PKG_LOG',
            p_function_name     => 'F_ECRIRE_LOG',
            ...
            p_nb_lignes         => n_Cnt,
            p_statut            => 'ERROR',
            p_message_erreur    => 'UTL_FILE.INVALID_PATH'
        );
        -- =====================================================================
        PKG_LOG.P_ECRIRE(t_InfoTrait,
                         PKG_LOG.gt_LOG_TYPE_ORA,
    ...
    WHEN  UTL_FILE.INVALID_MODE THEN
        ...
        SP_LOG_FICHIER(
            ...
            p_nb_lignes         => n_Cnt,
            p_statut            => 'ERROR',
            p_message_erreur    => 'UTL_FILE.INVALID_MODE'
        );
        -- =====================================================================
        PKG_LOG.P_ECRIRE(t_InfoTrait,
                         PKG_LOG.gt_LOG_TYPE_ORA,
    ...
    WHEN  UTL_FILE.INVALID_OPERATION THEN
        ...
        SP_LOG_FICHIER(
            ...
            p_nb_lignes         => n_Cnt,
            p_statut            => 'ERROR',
            p_message_erreur    => 'UTL_FILE.INVALID_OPERATION'
        );
        -- =====================================================================
        PKG_LOG.P_ECRIRE(t_InfoTrait,
                         PKG_LOG.gt_LOG_TYPE_ORA,
    ...
    WHEN  UTL_FILE.INVALID_FILEHANDLE THEN
        ...
        SP_LOG_FICHIER(
            ...
            p_nb_lignes         => n_Cnt,
            p_statut            => 'ERROR',
            p_message_erreur    => 'UTL_FILE.INVALID_FILEHANDLE'
        );
        -- =====================================================================
        PKG_LOG.P_ECRIRE(t_InfoTrait,
                         PKG_LOG.gt_LOG_TYPE_ORA,
    ...
    WHEN  UTL_FILE.WRITE_ERROR THEN
        ...
        SP_LOG_FICHIER(
            ...
            p_nb_lignes         => n_Cnt,
            p_statut            => 'ERROR',
            p_message_erreur    => 'UTL_FILE.WRITE_ERROR'
        );
        -- =====================================================================
        PKG_LOG.P_ECRIRE(t_InfoTrait,
                         PKG_LOG.gt_LOG_TYPE_ORA,
    ...
    WHEN  UTL_FILE.INTERNAL_ERROR THEN
        ...
        SP_LOG_FICHIER(
            ...
            p_nb_lignes         => n_Cnt,
            p_statut            => 'ERROR',
            p_message_erreur    => 'UTL_FILE.INTERNAL_ERROR'
        );
        -- =====================================================================
        PKG_LOG.P_ECRIRE(t_InfoTrait,
                         PKG_LOG.gt_LOG_TYPE_ORA,
    ...
    WHEN  NO_DATA_FOUND THEN
        ...
        SP_LOG_FICHIER(
            ...
            p_nb_lignes         => n_Cnt,
            p_statut            => 'ERROR',
            p_message_erreur    => 'NO_DATA_FOUND'
        );
        -- =====================================================================
        PKG_LOG.P_ECRIRE(t_InfoTrait,
                         PKG_LOG.gt_LOG_TYPE_ORA,
...
-- # DESCRIPTION  : Permet de concatener la clause WHERE d'une requete SQL
-- #                avec une autre condition en determinant
-- #                s'il faut ajouter un AND ou non
-- # PARAMETRES   : s_ClauseWhere : requete SQL
-- #                s_Condition   : condition a concatener
-- # MODIFICATIONS :
-- # ---------------------------------------------------------------------
-- # Version | Date       | Reference | Auteur | Specification
-- # ------- | ---------- | --------- | ------ | -------------------------
...
BEGIN
    -- Si le dernier mot de la requete en cours est WHERE alors la condition est la clause WHERE, on la concatene simplement
    IF INSTR(s_ClauseWhere, ' WHERE ')=(LENGTH(s_ClauseWhere)-6) THEN
       s_ClauseWhere := s_ClauseWhere||s_Condition;
    ELSE
    -- Sinon il y a deja une clause WHERE, il s'agit d'une nouvelle condition on ajoute donc AND avant
...
-- =======================================================================
-- # PROCEDURE    : F_EXT_CRE
-- # DESCRIPTION  : Genere un fichier de log au format CSV
-- #                et a partir des donnees de la table de log des messages
-- #                filtree en fonction des parametres fournis
-- # PARAMETRES   :
-- #    d_DateDebMsg   : date a partir de laquelle on commence
-- #                     a recuperer les logs
-- #    d_DateFinMsg   : date jusqu'a laquelle on arrete de recuperer les log
-- #    n_IdExec       : identifiant d'execution ou de declenchement
-- #                     du traitement
-- #    s_CdTypeMsg    : code du type de message ou liste de codes
-- #                     entre apostrophes et separes par une virgule
-- #    s_IdTraitement : idenfiant du traitement applicatif generateur
-- #                     du message
-- # MODIFICATIONS :
-- # ---------------------------------------------------------------------
-- # Version | Date       | Reference | Auteur | Specification
...
IS
    ...
    -- Nom des colonnes de la table a inclure en tete du fichier CRE
    s_NomChampTable VARCHAR2(128):=
        'SELECT ' ||
        '    ''DT_MESSAGE;LB_MESSAGE;ID_EXEC;CD_TYPE_MESSAGE;CD_EXCEPTION'' '||
        'FROM DUAL';

    -- Parametres de generation de la requete
    s_NomTableLog           VARCHAR2(30)   := 'LOG_MESSAGE';

    -- Liste des champs a extraire dans le CRE
    ...
    -- Clause WHERE : filtre sur l'identifiant d'execution
...
BEGIN
    ...
    -- Si aucun des 5 parametres n'existe
    ...
    THEN
        PKG_LOG.P_ECRIRE(t_InfoTrait,
                         PKG_LOG.gt_LOG_TYPE_ERR,
                         'Aucun des parametres necessaires au traitement ' ||
                         'n''est renseigne !'  ||
    ...
    -- Construire la requete qui va extraire le compte-rendu
    -- de la table les logs a partir des criteres en entree
    ...
    -- Construire la clause WHERE filtrant sur l'identifiant d'execution
    ...
    -- Ajout du tri a la requete pour avoir les messages dans l'ordre
    s_Query := s_Query||' '||s_QuerySortIdMessage;

    -- Completer le fichier CRE avec les donnees retournees par la requete
    n_CodeRet := F_ECRIRE_LOG(
                             t_InfoTrait,
                             s_Query,
                             s_Separator,
...
-- =======================================================================
-- # PROCEDURE    : F_INI_LST_TYPE_MSG_LOG
-- # DESCRIPTION  : Initialiser la liste des types de messages a tracer
-- # PARAMETRES   : t_InfoTrait : informations du traitement
-- # MODIFICATIONS :
-- # ---------------------------------------------------------------------
-- # Version | Date       | Reference | Auteur | Specification
-- # ------- | ---------- | --------- | ------ | -------------------------
...
IS
    ...
    -- Liste des types de messages de log qui sont traces par environnement
...
BEGIN
    ...
    -- Recuperation de l'environnement afin de determiner les logs a tracer
    n_CodeRet:=PKG_GLOBAL.F_GET_ENVIRONNEMENT(t_InfoTrait,s_Environnement);
    IF n_CodeRet <> PKG_GLOBAL.gn_CR_OK THEN
       RETURN PKG_GLOBAL.gn_CR_KO;
    END IF;

    -- Construction de la liste des types de log a tracer
    ...
    -- Mise a jour de la variable globale des types de log a tracer
...
/******************************************************************************
   ...
   PURPOSE: Ecrit le resultat de la requete passee en parametre, dans un
            table CLOB passe en parametre.
   PARAMS :  t_InfoTrait : Identifiant d'execution
            t_Param     : Identifiant de parametre
            ...
            c_Param     : Colonne d'identifiant de parametre dans t_Table
            c_Ordre     : Colonne d'identifiant d'ordre dans t_Table
            c_Texte    : Colonne de l'objet CLOB dans t_Table

   NOTE   : Cette fonction n'utilise PAS UTL_FILE (stockage CLOB uniquement)
            Pas d'instrumentation SP_LOG_FICHIER necessaire
